...
public void AI()
{
	...
	if(ticks%30 == 0){
		Main.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y,SoundHandler.soundID["onfire"]);
	}
	...
	foreach(NPC N in Main.npc)
    {
		...
		if(MB.Intersects(NB))
		{
			...
			N.StrikeNPC(45, 0f, 0, false, false, 2f);
		}
	}
}
public void PreKill()
{
	...
	if(Main.rand.Next(4) == 1) Main.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y,SoundHandler.soundID["blast"]);
}