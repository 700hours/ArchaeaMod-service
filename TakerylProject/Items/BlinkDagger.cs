...
namespace ArchaeaMod.TakerylProject.Items
{
	public class BlinkDagger : ModItem
	{
		...
		{
			Recipe.Create(Type)
				  .AddIngredient(57, 5)	// demonite bar
				  .AddIngredient(86, 10)	// shadow scale
			      .AddIngredient(2309, 5)	// specular fish
				  .AddTile(26)
				  .Register();
		}
		...
		public override void UpdateAccessory(Player player, bool hideVisual)
		{
			...
			if (midAir)
			{
				...
				if (ticks%6 == 0)
					airStrike = Projectile.NewProjectile(Item.GetSource_FromThis(), updateCenter, Vector2.Zero, 134, 64, Main.projectile[134].knockBack, player.whoAmI, 0f, 0f);
		...
		{
			bool Active = Main.tile[i, j-1].HasTile || Main.tile[i+1, j-1].HasTile;
			bool Solid = Main.tileSolid[Main.tile[i, j-1].TileType] == true || Main.tileSolid[Main.tile[i+1, j-1].TileType] == true;
		...
		{
			bool Active = Main.tile[i-1, j].HasTile || Main.tile[i-1, j+1].HasTile || Main.tile[i-1, j +2].HasTile;
			bool Solid = Main.tileSolid[Main.tile[i-1, j].TileType] == true || Main.tileSolid[Main.tile[i-1, j+1].TileType] == true || Main.tileSolid[Main.tile[i-1, j+2].TileType] == true;
