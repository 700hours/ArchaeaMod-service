    ...
    public class Broadsword : ModItem
    {
        ...
        public override void OnHitNPC(Player player, NPC target, int damage, float knockBack, bool crit)
        {
            if (crit)
            {
                ...
                {
                    Projectile proj = Projectile.NewProjectileDirect(target.Center, NPCs.ArchaeaNPC.AngleToSpeed(r, 6f), ModContent.ProjectileType<Pixel>(), item.damage, item.knockBack, player.whoAmI, Pixel.Fire, Pixel.Active);
                    proj.timeLeft = 20;
