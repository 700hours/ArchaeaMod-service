        ...
        public override void AI()
        {
            ...
            if (NPC.ai[0] >= 720)
            {
                ...
                if (Depreciate > 0 && sweep)
                {
                    ...
                    if (Depreciate % 10 == 0)
                    {
                        int attack = Projectile.NewProjectile(Projectile.GetSource_None(), NPC.position + new Vector2(NPC.width / 2, NPC.height / 2 + 32), Vector2.Zero, 134, 12 + Main.rand.Next(-2, 8), 2f, player.whoAmI, 0f, 0f);
                ...
                if (spinAttack)
                {
                    ...
                    {
                        int attack = Projectile.NewProjectile(Projectile.GetSource_None(), NPC.position + new Vector2(NPC.width / 2, NPC.height / 2 + 32), Vector2.Zero, 134, 20, 2f, player.whoAmI, 0f, 0f);
                        Projectile proj = Main.projectile[attack];
                        proj.velocity.X += TargetSD(null, NPC.rotation, 4f).X;
                        proj.velocity.Y += TargetSD(null, NPC.rotation, 4f).Y;
                        proj.friendly = false;
            ...
            if (!flames && Main.rand.Next(0, 4800) == 0)
            {
                for (int k = 0; k < 4; k++)
                {
                    ...
                    flamesID = NPC.NewNPC(NPC.GetSource_FromAI(), (int)nX, (int)nY, ModContent.NPCType<NPCs.PossessedBullet>());
                    if (Main.netMode != 0)
