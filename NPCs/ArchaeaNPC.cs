...
namespace ArchaeaMod.NPCs
{
    ...
    public class ArchaeaNPC
    {
        ...
        public static void DrawChainColored(Texture2D tex, SpriteBatch sb, Vector2 start, Vector2 end, Color color, int len = 12)
        {
            for (int n = 0; n < start.Distance(end); n += len)
            {
                double f = 1f;
                ...
                sb.Draw(tex,
                    new Vector2((float)cos, (float)sine) - Main.screenPosition,
                    new Rectangle(0, 0, len, (int)(len * f)), color, angle - Draw.radian * 90f, Vector2.Zero,
                    1f, SpriteEffects.None, 0f);
            }
        }
