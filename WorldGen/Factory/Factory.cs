...
namespace ArchaeaMod.Factory
{
    public class Factory
    {
        ...
        public void CastleGen(out ushort[,] tile, out ushort[,] background, int width, int height, int size = 8, int maxNodes = 50, float nodeDistance = 60)
        {
            ...
            foreach (Vector2 node in nodes)
            {
                ...
                switch (rand)
                {
                    case 0:
                        ...
                        r.bound = new Rectangle(X1 - border, X2 - border, W + border, H + border);
            ...
            for (int k = 1; k < nodes.Length; k++)
            {
                ...
                if (start.X < end.X && start.Y < end.Y)
                {
                    ...
                    {
                        CarveHall(brush, background, X, Y);
        ...
        private void CarveHall(ushort[,] tile, ushort[,] wall, int x, int y, int size = 10)
        {
            int border = 4;
